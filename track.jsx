/* Track screen — log and compare actual expenses vs budget */
...
function TrackScreen({ data, setData, monthIdx, setMonthIdx }) {
  ...
  /* Filter transactions to the selected year + month */
  ...
  const actualByCat = {};
  monthTxs.forEach(t => {
    actualByCat[t.categoryId] = (actualByCat[t.categoryId] || 0) + t.amount;
  });

  /* Flatten all categories, attach budgeted + actual */
  const cats = [];
  data.groups.forEach(g => g.cats.forEach(c => {
    ...
    if (budgeted > 0 || actual > 0) {
      cats.push({ ...c, groupId: g.id, groupName: g.name, budgeted, actual });
    }
  }));
  ...
  cats.forEach(c => {
    if (buckets[c.bucket]) { buckets[c.bucket].b += c.budgeted; buckets[c.bucket].a += c.actual; }
  });

  /* Mutations */
  const addTransaction = tx => setData(d => ({
    ...d, transactions: [...d.transactions, { id: uid(), ...tx }]
  }));
  const deleteTransaction = id => setData(d => ({
    ...d, transactions: d.transactions.filter(t => t.id !== id)
  }));

  /* Open modal for a specific category */
  const openModalForCat = (catId) => {
    setDefaultCatId(catId);
    setShowModal(true);
  };
  const handleModalClose = () => {
    setShowModal(false);
    setDefaultCatId(null);
  };
  ...
  return (
    <div>
      <div className="page-head">
        <div>
          ...
          <div className="sub">
            ...
            Click any category card or "Log expense" to track spending.
          </div>
        </div>
        <div className="actions">
          <button className="btn ghost"
            onClick={() => setMonthIdx((monthIdx + 11) % 12)}>‹ {MONTHS_LONG[(monthIdx + 11) % 12]}</button>
          <button className="btn ghost"
            onClick={() => setMonthIdx((monthIdx + 1) % 12)}>{MONTHS_LONG[(monthIdx + 1) % 12]} ›</button>
          <button className="btn ghost"
      ...
      <div className="kpis">
        ...
        <Kpi label="Left in budget"   value={remaining}
          tone={remaining < 0 ? 'neg' : (totAct > 0 ? 'pos' : undefined)} />
      ...
      {showTransactions ? (
        ...
        <div className="track-grid">
          {/* Left: category cards grouped by bucket */}
          <div>
            {cats.length === 0 ? (
              <div className="tx-empty">
                No budget categories set for this month. Add categories in the Budget tab, then log expenses here.
              </div>
            ) : (
              BUCKET_ORDER.map(b => {
                  ...
                  <div key={b} style={{ marginTop: 18 }}>
                    <div style={{
                      ...
                      <div style={{
                        ...
                        fontSize:11.5, textTransform:'uppercase', letterSpacing:'0.12em', color:'var(--ink-soft)',
                      }}>
                        <span className={'bucket-dot ' + b} style={{ margin:0 }}></span>
                        {BUCKETS[b].label}
                      </div>
                      <div style={{ fontSize:12.5, color:'var(--muted)', fontFamily:'var(--mono)' }}>
                        <span style={{ color:'var(--ink)' }}>{fmt(bs.a, { zero:'$0' })}</span>
                        {' '}of {fmt(bs.b, { zero:'$0' })} · {bp}%
                      </div>
                    </div>
          ...
          <div className="month-summary">
            ...
            {BUCKET_ORDER.map(b => (
              <div className="summary-row" key={b}>
                ...
                <span className="r">
                  {fmt(buckets[b].a, { zero:'$0' })}
                  <small> of {fmt(buckets[b].b, { zero:'$0' })}</small>
                </span>
              </div>
            ))}
...
function TransactionList({ transactions, data, onDelete, onAdd }) {
  const catMap = {};
  data.groups.forEach(g => g.cats.forEach(c => {
    catMap[c.id] = { name: c.name, bucket: c.bucket, group: g.name };
  }));
  ...
  if (!sorted.length) {
    return (
      <div className="tx-empty">
        ...
        <button className="btn primary" style={{ display:'inline-block', marginLeft:10 }}
          onClick={onAdd}>Log first expense</button>
      </div>
    );
  }
      ...
      {sorted.map(tx => {
        ...
        return (
          <div key={tx.id} className="tx-row">
            ...
            <span style={{ overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', color: tx.note ? 'var(--ink)' : 'var(--muted)' }}>
              {tx.note || '—'}
            </span>
            <span style={{ display:'flex', alignItems:'center', gap:8, overflow:'hidden' }}>
              {cat && (
                <span className={'bucket-dot ' + cat.bucket} style={{ flexShrink:0, margin:0 }}></span>
              )}
...
function CategoryCombobox({ data, value, onChange }) {
  ...
  const allCats = React.useMemo(() => {
    const list = [];
    data.groups.forEach(g => g.cats.forEach(c => {
      list.push({ id: c.id, name: c.name, group: g.name, bucket: c.bucket });
    }));
    return list;
  }, [data.groups]);
  ...
  const handleBlur = () => {
    /* Small delay lets click on list items register first */
  ...
  /* Render dropdown: grouped when no query, flat when filtering */
...
function LogExpenseModal({ data, monthIdx, year, defaultCategoryId, onSave, onClose }) {
  ...
  /* Set initial category — use defaultCategoryId if provided, else first cat */
  React.useEffect(() => {
    const catId = defaultCategoryId;
    if (catId) {
      setCategoryId(catId);
      for (const g of data.groups) {
        if (g.cats.find(c => c.id === catId)) { setGroupId(g.id); break; }
      }
  ...
  const handleCatChange = (id) => {
    setCategoryId(id);
    for (const g of data.groups) {
      if (g.cats.find(c => c.id === id)) { setGroupId(g.id); break; }
    }
  };
...
function CategoryCard({ cat, dayOfMonth, daysInMonth, onClick }) {
  const p       = cat.budgeted > 0
    ? Math.min(100, Math.round((cat.actual / cat.budgeted) * 100))
    : (cat.actual > 0 ? 100 : 0);
  ...
  return (
    <div
      className={'cat-card' + (over ? ' over' : '') + (unstarted ? ' unstarted' : '')}
      onClick={onClick}
      title="Click to log an expense for this category"
    ...
    >
      <div className="cat-head">
        <div className="name-c">
          <span className="dot" style={{ background: barColor }}></span>
          {cat.name}
        </div>
      ...
      <div className="bar">
        ...
        {cat.budgeted > 0 && (
          <div style={{
            position:'absolute', left: expectedPct + '%',
      ...
      <div className="meta">
        <span className="left">
          {unstarted ? 'click to log'
            : over   ? `${fmt(-remaining)} over`
            :          `${fmt(remaining)} left`}
        </span>
        <span>{p}%</span>
      </div>
